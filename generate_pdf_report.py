...
def create_pdf_report(timeline_path: Path, causal_path: Path, md_report_path: Path, pdf_output_path: Path):
    ...
    if md_report_path.exists():
        ...
        for line in lines:
            ...
            if in_code_block:
                code_lines.append(parse_markdown_to_html(line.rstrip()))
                continue
            
            # Header parsing
            if stripped.startswith("# "):
                heading_text = parse_markdown_to_html(stripped[2:])
                # Skip title if it is "Enterprise Network Incident Report" to avoid duplicates
                if "Incident Report" not in heading_text:
                    story.append(Paragraph(heading_text, h1_style))
            elif stripped.startswith("## "):
                heading_text = parse_markdown_to_html(stripped[3:])
                story.append(Paragraph(heading_text, h2_style))
            elif stripped.startswith("### "):
                heading_text = parse_markdown_to_html(stripped[4:])
                story.append(Paragraph(heading_text, h2_style))
            ...
            # Normal paragraph
                ...
                para_text = parse_markdown_to_html(stripped)
